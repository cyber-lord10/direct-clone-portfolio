  ...
  return (
    <section className="py-16 border-t border-border" aria-labelledby="social-proof-heading">
      <div className="container px-4">
        <FadeIn>
          <h2 id="social-proof-heading" className="font-display text-2xl md:text-3xl font-bold text-center mb-4">
          ...
          {platforms.map((platform, index) => (
            <FadeIn key={platform.name} delay={0.1 * index}>
              ...
              >
                ...
                <span className="font-display font-semibold text-foreground">{platform.name}</span>
                <span className="text-xs text-muted-foreground text-center">{platform.description}</span>
              </motion.a>
            </FadeIn>
          ))}
        </div>
